...
基于 [supergames #1](https://github.com/zidanDirk/supergames/issues/1) 实现的完整第一章节：Three.js 0.186、原生 ES Modules 和 Web Audio。立体潮玩球台、六种反弹器模型、四种怪物与章鱼 BOSS，支持 3D / 俯视视角切换。弹球碰撞仍在球台平面内计算，保留原有布阵规则。

奶油白、蓝紫、青柠与粉色的新界面，加入放置预览、选中后的旋转 / 回收面板、连击提示和独立 BOSS 血条。

## 运行

需要 Node.js 22 或更高版本，以及支持 WebGL 2 的浏览器：
...
Three.js 的压缩模块及许可证已随仓库提交，直接游玩无需安装依赖或构建；运行时不请求 CDN。开发与测试先运行 `npm ci`，重新生成引擎文件使用 `npm run vendor`。
...
- 两秒内连续击杀累积连击，5 / 10 / 15 次升至 ×2 / ×3 / ×4 得分倍率；无漏波额外奖励分数。
...
| 操作         | 桌面                                       | 手机 / 通用                               |
| ------------ | ------------------------------------------ | ----------------------------------------- |
| 放置         | 1–6 或点击选择库存，悬停预览后点击网格     | 选择库存后单击网格                        |
| 选中 / 视角  | 点击已有反弹器打开编辑面板；切换 3D / 俯视 | 同桌面                                    |
...
测试前运行 `npm ci`。125 项测试使用 Node 内置测试运行器，覆盖两种视角、四种尺寸下全部 126 个网格的射线拾取，以及 51 项物理测试、特效和库存规则、卡牌超时、BOSS 机制、存储容错以及 8 组固定随机种子的完整通关模拟。通关模拟使用固定布局，并等待选牌窗口接近结束后选择电击 / 分裂优先的卡牌。
...
PLAYWRIGHT_MODULE=/absolute/path/to/playwright/index.mjs node tests/scenes.mjs
# 只检查 WebKit
...
| `src/renderer.js`               | Three.js 模型、灯光、实例化粒子 / 拖尾、预警与伤害反馈 |
| `src/board-view.js`             | 正交相机、视角适配、网格射线拾取                       |
...
逻辑固定为 60Hz，渲染插值；碰撞子步将弹球每次位移限制在 4px 内。上限为 18 块反弹器、18 颗弹球、24 只怪物，粒子池固定 180 个。音效在用户首次操作后解锁，相邻 80ms 内的命中音合并节流。模型、贴图和图标均由程序生成。Three.js 使用 MIT 许可证，见 `vendor/three/LICENSE`。采用 Phong 材质与缓存贴地阴影；粒子和拖尾批量实例化渲染，手机渲染像素比上限为 1.5。
